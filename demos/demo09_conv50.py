"""Convection (beta=50) — u_t + 50*u_x = 0
...
pde = Convection(beta=50, device=device)
model = create_model(depth=2, width=10, activation='sin', device=device)
model, results = train(model, pde, lr=0.05, device=device)
results.update(pde='conv50', depth=2, width=10, activation='sin')
save_results(results, 'conv50')
plot_field(pde, model, 'conv50', device=device)
